...
async def create_user(
    user_input: Annotated[
        Optional[UserInput],
        Field(description="User object creation and update in store")
    ] = Body(...),
...
async def get_user_by_name(
    username: Annotated[
        StrictStr,
        Path(..., description="The username that needs to be fetched")
    ],
    db: AsyncSession = Depends(get_db),
) -> User:
    if not UsersService.subclasses:
...
async def update_user(
    username: Annotated[
        StrictStr,
        Path(..., description="The username to update")
    ],
    user_input: Annotated[
        Optional[UserInput],
        Body(..., description="User object creation and update in store")
    ],
    db: AsyncSession = Depends(get_db),
) -> None:
    if not UsersService.subclasses:
...
async def delete_user(
    username: Annotated[
        StrictStr,
        Path(..., description="The username to delete")
    ],
    db: AsyncSession = Depends(get_db),
) -> None:
    if not UsersService.subclasses:
